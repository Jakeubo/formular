...
<body class="bg-[url(http://localhost:8000/images/zap.webp)] bg-no-repeat bg-cover bg-center min-h-screen p-4 sm:p-6 flex items-start sm:items-center justify-center">

    <form id="main_form" method="POST" action="{{ route('order.store') }}"
        class="max-w-md w-full bg-white/80 backdrop-blur-md rounded-2xl shadow-xl space-y-5 font-sans">
        @csrf
        <input type="text" name="website" class="hidden">

        <div class="max-w-md w-full bg-white/80 backdrop-blur-md rounded-2xl shadow-xl p-6 space-y-5">
            ...
            <!-- Success message -->
            @if(session('success'))
            <div class="p-4 mb-4 text-sm rounded-xl bg-green-100 text-green-800 border border-green-300 shadow">
                {{ session('success') }}
            </div>
            @endif

            @if(session('error'))
            <div class="p-4 mb-4 text-sm rounded-xl bg-red-100 text-red-800 border border-red-300 shadow">
                ❌ {{ session('error') }}
            </div>
            @endif
            ...
            <!-- Jméno -->
            <div>
                <label for="first_name" class="block text-sm font-medium text-gray-700 mb-1">Jméno</label>
                <input type="text" name="first_name" id="first_name" placeholder="Jméno" required
                    class="w-full px-3 py-2 border border-[#E5B4D3] rounded-xl shadow-sm focus:border-pink-400 focus:ring-2 focus:ring-pink-200 transition">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label for="zip" class="block text-sm font-medium text-gray-700 mb-1">PSČ</label>
            ...
            <!-- Stát -->
            <div>
                <label for="country" class="block text-sm font-medium text-gray-700 mb-1">Stát</label>
            ...
            <div>
                <label for="listboxDopravce" class="block text-sm font-medium text-gray-700 mb-1">Dopravce</label>
                <select name="carrier" id="listboxDopravce" required
                    class="w-full px-3 py-2 border border-[#E5B4D3] rounded-xl shadow-sm bg-gradient-to-r from-pink-50 to-purple-50 
                       hover:from-pink-100 hover:to-purple-100 focus:border-pink-400 focus:ring-2 focus:ring-pink-200 transition">
            ...
            <div class="space-y-2">
                <p id="selectedPoint" class="hidden px-3 py-2 text-sm rounded-lg bg-indigo-100 text-indigo-800 font-medium"></p>
                <p id="balikovnaSelected" class="hidden px-3 py-2 text-sm rounded-lg bg-pink-100 text-pink-700 font-medium"></p>
                <p id="pplSelected" class="hidden px-3 py-2 text-sm rounded-lg bg-blue-100 text-blue-700 font-medium"></p>
            </div>
    ...
    <div id="pplModal" class="fixed inset-0 bg-black/60 items-center justify-center hidden z-50">
        <div class="relative bg-white rounded-2xl shadow-2xl w-[90%] h-[90%] p-6">
            <button id="closePpl" class="absolute top-4 right-4 text-gray-600 hover:text-gray-900 text-2xl font-bold">
                ✕
            </button>
            <h3 class="text-xl font-semibold mb-4">Vyberte výdejní místo PPL</h3>
            <div id="ppl-parcelshop-map" class="w-full h-[85%] border rounded-lg"></div>
        </div>
    </div>
